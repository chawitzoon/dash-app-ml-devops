...
def test_predict(mock_post, client):
    ...
    response = client.post(
        "/predict", json={"prices": [120.0, 125.0, 123.0, 127.0, 126.0, 128.0, 130.0]}
    )
    assert response.status_code == 200
    assert response.get_json() == {"result": 130.0}

    mock_post.assert_called_once_with(
        "http://127.0.0.1:8080/predict_next_price",
        json={"prices": [120.0, 125.0, 123.0, 127.0, 126.0, 128.0, 130.0]},
    )
